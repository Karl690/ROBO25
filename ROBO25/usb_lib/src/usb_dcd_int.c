...
#define TRYING_TO_STOP_THE_MADNESS
#ifdef TRYING_TO_STOP_THE_MADNESS
        // next 2 lines were added to clear interrupts in the case of not being connected properly
        // and the txfifo empty interrupt kept firing
        fifoemptymsk = 0x1 << epnum;
        USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, fifoemptymsk, 0);
#endif
...
static uint32_t DCD_WriteEmptyTxFifo(USB_OTG_CORE_HANDLE *pdev, uint32_t epnum)
{
  ...
  len32b = (len + 3) / 4;
  txstatus.d32 = USB_OTG_READ_REG32( &pdev->regs.INEP_REGS[epnum]->DTXFSTS); //DEBUG_GB
